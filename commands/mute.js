...
module.exports = {
    ...
    run: async function(client, message, args){
        ...
        return message.reply(`<:no:884059056174682122> You can't mute your own self!`).catch(err => log.send(`\`\`\`fix\nGUILD - ${message.guild.name} ERROR - missing permission\`\`\``));


        if (user.id === client.user.id)
        return message.reply(`Please Don't mute Me :(`).catch(err => log.send(`\`\`\`fix\nGUILD - ${message.guild.name} ERROR - missing permission\`\`\``));
        ...
        if(!muterole) {return message.reply("<:no:884059056174682122> This server do not have mute role, use `-muterole <@mute role>` to set the mute role!").catch(err => log.send(`\`\`\`fix\nGUILD - ${message.guild.name} ERROR - missing permission\`\`\``));}
        ...
        if(user.roles.cache.has(muterole.id)) {return message.reply("<:no:884059056174682122> Given user is already muted!").catch(err => log.send(`\`\`\`fix\nGUILD - ${message.guild.name} ERROR - missing permission\`\`\``));}
...
.setDescription(`<:success:883599837789036545> **${user.user.tag} has been muted.**`)
.setColor('RED') 
message.channel.send({ embeds : [embed]}).catch(err => log.send(`\`\`\`fix\nGUILD - ${message.guild.name} ERROR - missing permission\`\`\``));

let embed2 = new MessageEmbed()
.setDescription(`You were muted in ${message.guild.name} Reason: ${reason} By: ${message.author.tag}`)
.setColor(`RED`)
